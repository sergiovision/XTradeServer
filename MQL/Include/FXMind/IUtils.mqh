...
public:
   ...
   virtual IFXMindService* Service() { return NULL; }
   ...
   virtual bool  OrderModify(long  ticket, double price, double stoploss, double takeprofit, datetime expiration, color arrow_color) {return false;}
   ...
   virtual void AddToChart(int Handle, string IndicatorName, long chartID, int subWin)
   {
    ...
    double PercentileATR(string sym, ENUM_TIMEFRAMES tf, double rank, int atrPeriod, int shift)
    {
         MqlRates rates[]; 
         ArraySetAsSeries(rates,true); 
         int copied = CopyRates(sym, tf, shift, atrPeriod, rates);
         if (copied > 0)
         {
            double candles[];
    ...
    double PercentileATRIndi(ENUM_TIMEFRAMES tf, const double& low[], const double& high[], double rank, int atrPeriod, int shift)
    {
         double candles[];
         int barsPerPeriod = (int)BarsPerPeriod(tf);
         if (barsPerPeriod == 1)
         {
            int arraySize = (int)atrPeriod*barsPerPeriod;
            ...
            return PercentileATR(Symbol(), tf, rank, atrPeriod, shift); 
