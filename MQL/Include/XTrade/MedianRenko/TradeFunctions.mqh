...
#include <XTrade/MedianRenko/Normailze.mqh>
...
{  
   ...
   while(!IsStopped() && !result)
   {
      ...
      {
         Utils.Info("Unable to place trade: "+tradingChecks.GetCheckErrorToString());
         return false;
      }
...
//      priceTP = (takeprofit ? NormalizePrice(symbol,price - takeprofit*_point) : PositionGetDouble(POSITION_TP));
   ...
   while(!IsStopped() && !result)
   {
      ...
      {
         Utils.Info("Unable to modify: "+tradingChecks.GetCheckErrorToString());
         return false;
      }
