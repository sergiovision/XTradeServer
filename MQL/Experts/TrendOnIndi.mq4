  ...
  if( id == CHARTEVENT_KEYDOWN )
  {
      ...
      if ((lastChar=='g') && (lparam=='s' || lparam=='S')) { 
         ...
         if (ticket != -1) {      
            ...
            double SL = NormalizeDouble(Ask + StopLossLevel * Point , Digits); //* currentImportance
...
void CreateTextLabel(string msg, int Importance, datetime raisetime) 
{
   ...
   if (!IsTesting() || IsVisualMode()) {
      string name = "newsevent" + (string)MathRand();
...
{   
   ...
   if (!IsTesting() || IsVisualMode())
   {
      ...
      globalComment += "In News period: " + (string)InNewsPeriod + "\n";
      ...
      globalComment += "Trend: " + trendString + (string)prevIndiSignal + raisedStr + message + "\n";
   }
...
void AddSpreadComment()
{
   if (!IsTesting() || IsVisualMode()) {
      ...
      globalComment = globalComment + "Current spread: " + (string)spread + "\n";
   }
}
//+------------------------------------------------------------------+
void ShowComment() {
...
int OnInit()
{
   ...
   string initMessage = "OnInit GridTrendOnIndi Magic: " + IntegerToString(Magic) + " Digits: " + (string)Digits;
   Print(initMessage);
   ...
   if ((AllowSELL == false)&& op_type==OP_SELL)
    ...
    while (!exit_loop) 
    {
       ...
       else 
       {
          if (err != ERR_NO_ERROR) 
          {
             globalComment += "^^^^Error Opening Order ticket:  " + (string)ticket + ", error#: " + (string)err + "\n";
          }
       }
       if (err == ERR_NO_ERROR) 
       {
       ...
       string err_str = "Still error after " + (string)RetryOnErrorNumber + " retries!!! " + TimeToStr(TimeCurrent(),TIME_MINUTES);
...
bool ChangeOrder(int ticket, double price, double stoploss, double takeprofit, datetime expiration, color arrow_color)
{
   if (OrderSelect(ticket, SELECT_BY_TICKET))
   {
      ...
      while (!exit_loop)
      {
         ...
         else 
         {
            if (err != ERR_NO_ERROR) 
            {
               Print("^^^^^^^^^^Error OrderModify ticket: " + (string)ticket + ", error#: " + (string)err);
            }
         }
      }
      return result;
   ...
   {
      Print("Unable to select Order ticket: " + (string)ticket);
      return false;
   }
}
//+------------------------------------------------------------------+
...
bool CloseOrder(int ticket, double lots, double price, int slippage, color arrow_color)
{
   if (OrderSelect(ticket, SELECT_BY_TICKET))
   {
      ...
      while (!exit_loop)
      {
         ...
         else 
         {
            if (err != ERR_NO_ERROR) 
            {
               Print("^^^^^^^^^^Error Close Order ticket: " + (string)ticket + ", error#: " + (string)err);
            }
         }
      }
      return result;
   ...
   {
      Print("Unable to Close Order by ticket: " + (string)ticket);
      return false;
   }
}
//+------------------------------------------------------------------+
...
double CalculateLotSize(int op_type, double prevLot)
{
    ...
    if ( EnableSentimentsLotSize && (sentimentsMultiplier > 1) )
       Print ( ">>>>>>>>>>>>>>> Increase multiplier: " + (string)lotSize);
    return lotSize;     
}
...
{
   double lots = OrderLots();
   Print("!!!Close expired order: " + (string)Ticket);
   double closePrice = 0;
   if (OP_BUY == OrderType())
      closePrice = Bid;
   else 
...
bool ProcessOrders()
{
   ...
   if ((op_type == OP_BUY) && (countBUY == 0) && (grid_optype == -1) ) 
   {
      ...
      if (ticket != -1) {
         ...
         SL = NormalizeDouble(Ask - StopLossLevel * Point , Digits); //* currentImportance
         OrderModify(ticket, OrderOpenPrice(), SL, TP, 0);        
         EventRaiseSoon = false;
         return true;
      }
         ...
         if ( (tip < 2) && (OrderSymbol()==Symbol()) && (OrderMagicNumber()==Magic)) // && CalcOrderRealProfit() >= 0)
         {
            ...
            {  
               ...
               if (StLo > OSL || OSL==0)
               {
                  ...
                  globalComment += "TrailingStop " + (string)Ticket + " " + TimeToStr(TimeCurrent(),TIME_MINUTES) + "\n";
                  break;
               } 
            }                                         
